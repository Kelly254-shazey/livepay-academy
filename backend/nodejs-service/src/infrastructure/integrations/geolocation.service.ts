...
export class GeolocationService {
   ...
   */
  async detectLocation(ipAddress: string): Promise<GeoLocationData | null> {
    try {
      // In production, use MaxMind GeoIP2, IP2Location, or similar service
      // For now, mock implementation
      const response = await fetch(
        `https://ip-api.com/json/${ipAddress}?fields=country,city,regionName,lat,lon,timezone,countryCode`
      );
   ...
   */
  isPaymentMethodAvailable(countryCode: string, method: string): boolean {
