...
class AdminRepository {
    ...
    reviewCreator(creatorUserId, reviewerId, decision, notes) {
        return this.db.$transaction(async (tx) => {
            ...
            const moderationAction = await tx.moderationAction.create({
                data: {
                    ...
                    metadata: (0, prisma_json_1.toPrismaNullableJson)({
                        creatorUserId
                    })
                }
            });
    ...
    suspendUser(userId, moderatorId, reason) {
        return this.db.$transaction(async (tx) => {
    ...
    async suspendResource(resourceType, resourceId, moderatorId, reason) {
        return this.db.$transaction(async (tx) => {
            let resource;
            switch (resourceType) {
                case "live_session":
